function graph2ply(PLYfilename, coordinate, edges, rgb)
    % coordinate is 3 * n single matrix for n points
    % edges      is 2 * m matrix for m triangles
    ...
    % write faces data
    edges_data = int32(edges-1);
    edges_data = reshape(typecast(reshape(edges_data,1,[]),'uint8'),2*4,[]);        
    fwrite(file, edges_data,'uint8');
