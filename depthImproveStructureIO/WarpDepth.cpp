...
% mex WarpDepth.cpp -L/opt/X11/lib/ -I/opt/X11/include/ -lOSMesa
% or
% mex WarpDepth.cpp -lGLU -lOSMesa -I/media/Data/usr/Mesa-9.1.2/include
% on mac:
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  double threshold_squared = Square(mxGetScalar(prhs[4])); // meter
  
  mexPrintf("threshold^2:%f meter^2\noutput size:\nm_width=%d\nm_height=%d\n", threshold_squared, m_width,m_height);
 ...
 // double inv_height_scale = 1.0/(m_height*scale);
  ...
  //double inv_width_scale_2 = inv_width_scale*2.0;
  //double inv_height_scale_2_s = -inv_height_scale*2.0;
double cx_rgb = projection[6]; //m_width/2;
double cy_rgb = projection[7]; //m_height/2;
double inv_width_scale_1 = 1.0 - 4 * cx_rgb / m_width;
double inv_height_scale_1_s = -1.0 + 4 * cy_rgb / m_height;
double inv_width_scale_2 = 2.0 / m_width;
double inv_height_scale_2_s = -2.0 / m_height;
  ...
  for (unsigned int c = 0; c < num_cols-1; ++c) {
    ...
    for (unsigned int r = 0; r < num_rows-1; ++r) {
        ...
        if (*l00 == 0.0){
            if ( *l01 != 0.0 && *l10 != 0.0 && *l11 != 0.0 
                 && Square(*x01-*x11)+Square(*y01-*y11)+Square(*z01-*z11) < threshold_squared 
                 && Square(*x01-*x10)+Square(*y01-*y10)+Square(*z01-*z10) < threshold_squared 
                 && Square(*x10-*x11)+Square(*y10-*y11)+Square(*z10-*z11) < threshold_squared
            ){
            ...
            if (*l11 == 0.0){
                if (*l01!= 0.0 && *l10 != 0.0
                    && Square(*x01-*x00)+Square(*y01-*y00)+Square(*z01-*z00) < threshold_squared 
                    && Square(*x01-*x10)+Square(*y01-*y10)+Square(*z01-*z10) < threshold_squared 
                    && Square(*x10-*x00)+Square(*y10-*y00)+Square(*z10-*z00) < threshold_squared
                ){
                ...
                if (*l01!=0.0
                    && Square(*x01-*x11)+Square(*y01-*y11)+Square(*z01-*z11) < threshold_squared 
                    && Square(*x01-*x00)+Square(*y01-*y00)+Square(*z01-*z00) < threshold_squared 
                    && Square(*x00-*x11)+Square(*y00-*y11)+Square(*z00-*z11) < threshold_squared
                ){
                    uint2uchar(static_cast<unsigned int>(*l00),colorBytes);
                    glColor3ubv(colorBytes);
                    glBegin(GL_TRIANGLES);
                ...
                if (*l10!=0.0
                    && Square(*x00-*x11)+Square(*y00-*y11)+Square(*z00-*z11) < threshold_squared 
                    && Square(*x00-*x10)+Square(*y00-*y10)+Square(*z00-*z10) < threshold_squared 
                    && Square(*x10-*x11)+Square(*y10-*y11)+Square(*z10-*z11) < threshold_squared
                ){
                    uint2uchar(static_cast<unsigned int>(*l00),colorBytes);
                    glColor3ubv(colorBytes);
